module Fluent
  class ConcatFilter < Filter
    ...
    def initialize
      ...
      @regexp_cache = Hash.new
      @is_regexp = Regexp.compile(/^\/.+\/$/)
    end
    ...
    def firstline?(record, key)
      ...
      if record.has_key? @regexp_override_key and !!@is_regexp.match(record[@regexp_override_key])
        override_regexp_val = record[@regexp_override_key][1..-2]
        if @regexp_cache.has_key? override_regexp_val
          start_regexp = @regexp_cache[override_regexp_val]
        else
          start_regexp = Regexp.compile(record[@regexp_override_key][1..-2])
          @regexp_cache[override_regexp_val] = start_regexp
        end
      end
